"""`chunksim search`: the whole world, not just what this map can reach.

`derive/search.py` walks all five item routes over the raw export, so this is
a strict superset of what `chunksim sources` can list - which is the point, since
...
from chunksim.cli.common import derive_cached, emit_json, load_state
from chunksim.derive.task_names import strip_task_markup
from chunksim.derive.search import TYPES, build_world_index, search
