"""`chunksim unlock`: what one candidate chunk would add.

Additions only, and only over one `MapState` - `derive/unlock.py` owns the
attribution rule and its one exception. For two arbitrary maps, `chunksim diff` is
...
from chunksim.cli.common import derive_cached, digests, emit_json, load_state
from chunksim.derive.unlock import UnlockDelta, tasks_added_by
from chunksim.runs.batch import save_unlock
...
def _cmd_unlock(args: argparse.Namespace) -> int:
    ...
    if args.export_json != "-":
        ...
        if saved is not None:
            ...
            print(f"read with    chunksim tasks --map {saved}")
