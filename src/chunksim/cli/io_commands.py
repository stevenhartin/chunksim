...
def _cmd_heuristics(args: argparse.Namespace) -> int:
    """`chunksim heuristics`: rebuild the scraped half of the estimator's numbers.

    Thirty-odd requests, run about as often as `chunksim chunkinfo`. `scrape.py` does the
...
def _cmd_recipes(args: argparse.Namespace) -> int:
    """`chunksim recipes`: what one action of a training method pays and costs.

    A dozen Bucket queries, one per skill that has recipes. Separate from
    `chunksim heuristics` because it fetches different things from a different API
...
def add_arguments(
    ...
    # product is the cache blob, and `chunksim estimate --export-json` is how the
