"""`chunksim simulate`: roll chunks from this map's state, N times over.
...
from chunksim.cli.common import digests, emit_json, error, load_state
from chunksim.runs.batch import RunResult, run_batch
from chunksim.runs.simulate import simulate_rolls
from chunksim.store.cache import read_cache
from chunksim.store.derived_cache import CacheBehaviour, RollCache


def _simulate_to_cache(args: argparse.Namespace) -> int:
    """`chunksim simulate --cache-map`: persist each run as its own cached map."""
    ...
    if not quiet:
        ...
        print(f"read with    chunksim tasks --map {read_as}")
