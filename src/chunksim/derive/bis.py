...
claims when that wins, so a strictly worse item can be the right pick: the
second cached map records `toktz-xil-ak` for Melee weapon over an abyssal whip that beats it 82/82
...
class SetEffect:
    ...
    *replace* the slots it claims when that number wins. Without it the
    ...
    holding the obsidian set is visibly not upstream's answer: the second
    cached map records `toktz-xil-ak` for Melee weapon where the abyssal whip beats it 82/82
...
#: map exercises, so it is the one that can be checked against an
