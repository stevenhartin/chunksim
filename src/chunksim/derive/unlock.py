...
from chunksim.derive.delta import diff_nested, diff_picks
...
def newly_trainable_backlog(
    ...
    **The one thing an unlock adds that `diff_valid_tasks` cannot see.**
    ...
    Measured, on a simulated run of the second cached map: rolling `12849`
    adds Chaeldar, flipping
...
def _clamped_levels(
    ...
    **The ledger records the export's `Level` and the panel ranks on it, but
    ...
    Measured, on a simulated run of the second cached map rolling `5179`
    with `rules['Boosting']`
...
def tasks_added_by(
    ...
    *,
    ...
    (`derived_cache.cached_derive`) - two derives is why `chunksim unlock` costs
