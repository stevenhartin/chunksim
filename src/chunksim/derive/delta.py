...
makes this branch agree with what `chunksim sources` prints, which is the point
...
from chunksim.derive.active_tasks import SkillClassification
from chunksim.derive.other_tasks import CategoryTasks
...
#: The comparable branches, in the order `chunksim diff` reports them. `bis`
...
#: isn't listable by `chunksim sources`) but it does change between states, and
...
class StateDelta:
    ...
    def counts(self) -> dict[str, tuple[int, int]]:
        """`(added, removed)` per branch, for the summary `chunksim diff` prints.
