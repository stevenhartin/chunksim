...
already run `chunksim tasks` and vice versa.
"""
...
from chunksim.costing.inputs import ReferenceBlobs, load_reference
...
class Derivations:
    ...
    def reference(self, map_id: str | None = None) -> ReferenceBlobs:
        ...
        **The one memo here that can go stale into a wrong answer.** The
        export is replaced by an explicit `chunksim chunkinfo`, which calls
    ...
    def reset(self) -> None:
        ...
        Called when `chunksim chunkinfo` refreshes the file underneath us. The
    ...
    def load(self, map_id: str) -> DerivedState:
        ...
        `chunksim fetch` landing under it, which is the invalidation problem this
