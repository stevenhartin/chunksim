...
from chunksim.derive.delta import MapSide
...
def reachable_by_area(state: DerivedState) -> dict[str, Any]:
    ...
    **There is no route data to find them by.** The obvious mechanism would be
    ...
    Measured: 46 squares on the second map and 92 on the reference map, of
    which 31 and 56 are in
    the northern block.
        ...
        "chunks": [
            {
                ...
                # one cached map; naming one would be picking a favourite.
...
def _full_diff(map1: str, map2: str, ctx: Context) -> dict[str, Any]:
    """`chunksim diff --map1 --map2`, over every branch.
