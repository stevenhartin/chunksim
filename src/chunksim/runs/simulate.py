...
from chunksim.derive.graph import SectionGraph, build_section_graph
from chunksim.derive.neighbours import neighbour_pool
...
def simulated_payload(
    ...
    *,
    ...
    - `chunkOrder` gains one `{timestamp_ms: chunk_id}` entry per roll, its
      ...
      but leaving it alone would make `chunksim show` misreport a simulated map.
    ...
    **Two of those four branches make a run's `MapState` hash differently from
    ...
    - `chunkinfo.activeTasks` and `chunks.selected` are **dropped**. They hold
      ...
      `chunksim tasks`'s "upstream says" line would compare against it.
    """
