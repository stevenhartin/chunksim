...
installed" and exits 0 - so the `fray` on `PATH` can be last week's wheel while
...
kill `fray tasks` would be a bad trade for a line of provenance.
"""
...
#: which `python -m fray_claude` reaches without installing anything.
KINDS = ("wheel", "editable", "source")


@dataclass(frozen=True)
class Build:
    """Where this process's `fray_claude` came from, and when."""
...
NO_WATERMARK_ENV = "FRAY_NO_WATERMARK"
...
def print_watermark(app: str, stream: TextIO | None = None) -> None:
    ...
    **stderr by default, and that is not a detail**: nine `fray` subcommands
    can write their whole answer to stdout with `--export-json -`, and a line
    of provenance in front of it turns valid JSON into a parse error. `fray-gui`
