"""Tests for `costing/brimstone.py` - see its module docstring for the
mechanic, the wiki formula it ports and what it deliberately does not model."""

from __future__ import annotations

import pytest

from chunksim.costing import brimstone
...
class TestKeyDropChance:
    def test_the_wikis_own_worked_seam(self) -> None:
        # https://oldschool.runescape.wiki/w/Brimstone_key - both pieces of
        # the formula agree exactly at the L=100 seam.
        assert brimstone.key_drop_chance(99.0) == pytest.approx(
            1.0 / (100 + int((100 - 99) ** 2 / 5))
        )
        assert brimstone.key_drop_chance(100.0) == pytest.approx(1.0 / 100.0)
        assert brimstone.key_drop_chance(101.0) == pytest.approx(1.0 / (120 - 20))

    def test_capped_at_1_50_from_level_350(self) -> None:
        assert brimstone.key_drop_chance(350.0) == pytest.approx(1.0 / 50.0)
        assert brimstone.key_drop_chance(1000.0) == pytest.approx(1.0 / 50.0)

    def test_an_unknown_level_is_zero_not_a_guess(self) -> None:
        assert brimstone.key_drop_chance(0.0) == 0.0
        assert brimstone.key_drop_chance(-5.0) == 0.0
...
def _info(
    ...
    """See `larran._info` - the same fixture shape, kept separate here since
    `tests/` is not a package and cannot import it from that other file."""
    return ChunkInfo(
        {
...
class TestRequiresSlayerLevel:
    def test_a_bare_name_matches(self) -> None:
        ...
        assert brimstone._requires_slayer_level(info, "Abyssal demon")

    def test_a_variant_suffixed_name_still_matches_the_bare_monster(self) -> None:
        info = _info({"Cave bug#Level 96": 7})
        assert brimstone._requires_slayer_level(info, "Cave bug")

    def test_an_unlisted_monster_does_not(self) -> None:
        info = _info({"Abyssal demon": 85})
        assert not brimstone._requires_slayer_level(info, "Cow")


class TestMonsterKeyChance:
    def test_the_slayer_bonus_is_a_flat_20_percent(self) -> None:
        info = _info({"Abyssal demon": 85})
        heuristics = Heuristics(
            monster_stats={"Abyssal demon": MonsterStats(name="Abyssal demon", hitpoints=150, combat_level=124)}
        )
        chance = brimstone._monster_key_chance(info, heuristics, "Abyssal demon")
        assert chance == pytest.approx(brimstone.key_drop_chance(124.0) * 1.20)
    ...
    def test_no_slayer_requirement_gets_no_bonus(self) -> None:
        ...
        chance = brimstone._monster_key_chance(info, heuristics, "Cow")
        assert chance == pytest.approx(brimstone.key_drop_chance(2.0))
    ...
    def test_no_stated_combat_level_is_none_not_zero(self) -> None:
        ...
        assert brimstone._monster_key_chance(info, heuristics, "Ghost") is None

    def test_an_unscraped_monster_is_none(self) -> None:
        assert brimstone._monster_key_chance(_info(), Heuristics(), "Nothing") is None


def _konar(*tasks: TaskRate) -> MasterRate:
    return MasterRate(master="Konar quo Maten", xp_per_hour=0.0, tasks=tasks)
...
class TestKeysPerHour:
    def test_a_single_task_matches_hand_arithmetic(self) -> None:
        ...
        rate = brimstone.keys_per_hour(master, info, heuristics, frozenset({"Abyssal demon"}))

        chance = brimstone.key_drop_chance(124.0) * 1.20
    ...
    def test_weighted_across_two_tasks_by_time_not_by_count(self) -> None:
        ...
        master = _konar(cows, jellies)

        rate = brimstone.keys_per_hour(master, info, heuristics, frozenset({"Cow", "Jelly"}))

        cow_chance = brimstone.key_drop_chance(2.0)
        jelly_chance = brimstone.key_drop_chance(61.0)
    ...
    def test_an_unreachable_monster_contributes_nothing(self) -> None:
        ...
        assert brimstone.keys_per_hour(master, info, heuristics, frozenset()) == 0.0

    def test_no_tasks_at_all_is_zero(self) -> None:
        master = _konar()
        assert brimstone.keys_per_hour(master, _info(), Heuristics(), frozenset()) == 0.0

    def test_a_superior_spawn_adds_a_guaranteed_key_on_top(self) -> None:
        """"A brimstone key is always dropped by a superior slayer monster
        assigned by Konar" - `slayer.superior_spawns_per_hour` is exactly
        that rate, added flat."""
        ...
        without_superior = brimstone.keys_per_hour(
            master, info, Heuristics(monster_stats=heuristics.monster_stats), frozenset({"Cow"})
        )
        with_superior = brimstone.keys_per_hour(master, info, heuristics, frozenset({"Cow"}))
...
class TestEffectiveSeconds:
    def test_adds_the_open_action_on_top_of_the_key(self) -> None:
        got = brimstone.effective_seconds(2.0)  # one key every half hour
        assert got == pytest.approx(1800.0 + brimstone.OPEN_SECONDS)

    def test_a_non_positive_rate_is_none(self) -> None:
        assert brimstone.effective_seconds(0.0) is None
        assert brimstone.effective_seconds(-1.0) is None


class TestPriced:
    def test_no_master_is_a_no_op(self) -> None:
        heuristics = Heuristics()
        assert brimstone.priced(heuristics, None, _info(), frozenset()) is heuristics

    def test_the_chest_gets_a_real_derived_rate(self) -> None:
        info = _info(
            {"Abyssal demon": 85},
            slayer_tasks={"Abyssal demons": {"Abyssal demon": True}},
        )
        ...
        result = brimstone.priced(heuristics, master, info, frozenset({"Abyssal demon"}))

        rate = result.kills_per_hour(brimstone.CHEST)
        assert rate.value > 0
        assert not rate.source.startswith("default")
    ...
    def test_no_reachable_candidate_still_writes_an_explicit_zero(self) -> None:
        ...
        result = brimstone.priced(Heuristics(), master, _info(), frozenset())

        assert brimstone.CHEST in result.monsters
        assert result.monsters[brimstone.CHEST].value == 0.0
