...
def test_reference_stamp_moves_when_a_blob_is_written(tmp_path: Path) -> None:
    ...
    # One entry per reference file: the rate scrape, the recipes, the aliases,
    # the courier and bounty tables, and the checked-in overrides. A missing
    # file stamps as `(0, 0)`, so appearing or vanishing both move it.
    assert empty == ((0, 0),) * 6, "nothing on disk stamps as absent"
